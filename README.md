...
- 本地缓存：用 Room 数据库存结果并记录 prompt 版本，查过的词秒开，prompt 升级后自动重新生成
- 历史记录按时间倒序排列，点开就能重新查看
- 收藏值得复看的卡片，可以补充自己的笔记
- 把概念卡导出成文本，方便保存或发给别人
- BYOK（Bring Your Own Key）：API Key 只存在设备本地，支持任意 OpenAI 兼容端点
- 基于 Material Design 3 和 Compose，支持深色模式

## 当前状态

项目处于 RC 前的收口阶段。核心流程已经可用：查询、卡片、历史、收藏、笔记、复制、重新生成。Prompt 升级到了 v3，支持单词、固定短语和普通词组。GitHub Actions 会构建 debug 和 release APK，release signing 也已接入，但要出正式签名包还需要在仓库里配置 signing secrets。

暂时不做的：登录、推荐算法、SRS 复习、阅读器、社区、离线词典。
